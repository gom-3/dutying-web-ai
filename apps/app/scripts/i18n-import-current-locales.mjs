...
const rows = runtimeKeys.map((runtimeKey) => ({
    ...
    displayPolicy: '',
    maxLength: '',
    emptyLocales: ['ko', 'en', 'ja', 'zh', 'th'].filter((locale) => flatLocales[locale][runtimeKey] === '').join('|'),
    termIds: '',
    context: '',
    route: '',
    component: '',
...
const termHeaders = ['termId', 'ko', 'en', 'ja', 'zh', 'th', 'description', 'forbiddenEn', 'forbiddenJa', 'forbiddenZh', 'forbiddenTh'];
const seedTerms = [
    {
        termId: 'ward',
        ko: '병동',
