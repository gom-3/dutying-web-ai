...
근무표 입력 단계(3단계)에서 한국어 UI일 때만 제목의 마지막 문구 바로 뒤에 설명 영상 버튼을 표시한다. 버튼은 검정 배경에 양끝이 둥근 형태이며, 흰색 재생 아이콘과 `설명 영상 · 1분` 문구를 사용한다. `1분`은 요청한 버튼 표시 문구이고 실제 영상 길이는 약 1분 53초다. 버튼을 누르면 화면 중앙의 영상 모달을 열고, 배경은 어둡게 흐리게 처리한다. 원래 근무표의 위치와 입력 내용은 유지하며, 모달이 열린 동안에는 배경 스크롤과 포커스 이동을 막는다. 닫기 버튼, 배경 클릭, Escape로 닫으면 설명 영상 버튼에 포커스를 돌려준다.

버튼을 누르기 전에는 영상·포스터를 요청하지 않는다. 닫기, 단계 이동, 언어 변경 시 재생을 중지하고 리소스를 해제한다. 작은 화면이나 가로 화면에서도 영상이 잘리지 않도록 화면 높이에 맞추고 원본 비율을 유지한다.
