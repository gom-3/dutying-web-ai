...
근무표 입력 단계(3단계)에서 한국어 UI일 때만 제목 오른쪽에 설명 영상 버튼을 표시한다. 버튼을 누르면 화면 중앙의 영상 모달을 열고, 배경은 어둡게 흐리게 처리한다. 원래 근무표의 위치와 입력 내용은 유지하며, 모달이 열린 동안에는 배경 스크롤과 포커스 이동을 막는다. 닫기 버튼, 배경 클릭, Escape로 닫으면 설명 영상 버튼에 포커스를 돌려준다.
