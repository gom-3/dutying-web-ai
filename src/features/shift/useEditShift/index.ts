...
import {checkShift, findNurse, keydownEventMapper, moveFocus, updateCheckFaultOption} from './handlers';
...
const useEditShift = (activeEffect = false) => {
    ...
    const {data: shift, status: shiftStatus} = useQuery({
        ...
        queryFn: async () => {
            const res = await WardAPI.getShift(wardId!, currentShiftTeamId!, year, month);
    ...
    const {mutate: mutateShift, status: changeStatus} = useMutation({
        mutationFn: ({wardId, focus, shiftTypeId}: {wardId: number; focus: Focus; shiftTypeId: number | null}) =>
            WardAPI.updateShift(wardId, year, month, focus.day + 1, focus.shiftNurseId, shiftTypeId),
            ...
            queryClient.setQueryData<Shift>(
                ...
                produce(oldShift, (draft) => {
                    draft.divisionShiftNurses
                        .flatMap((x) => x)
                        .find((x) => x.shiftNurse.shiftNurseId === focus.shiftNurseId)!.wardShiftList[focus.day] = shiftTypeId;
                }),
            );
    ...
    const {mutate: mutateShiftsAndHistory} = useMutation({
        mutationFn: ({wardId, wardShiftsDTO}: {wardId: number; wardShiftsDTO: WardShiftsDTO; lastFocus: Focus; diff: number}) =>
            WardAPI.updateShifts(wardId, wardShiftsDTO),
    ...
    const {mutate: postShiftMutate, isPending: postShiftLoading} = useMutation({
        mutationFn: ({wardId, shiftTeamId, year, month}: {wardId: number; shiftTeamId: number; year: number; month: number}) =>
            WardAPI.postShift(wardId, shiftTeamId, year, month),
    });
    ...
    const changeFocusedShift = useCallback(
        (shiftTypeId: number | null) => {
            ...
            if (
                shift.divisionShiftNurses.flatMap((x) => x).find((x) => x.shiftNurse.shiftNurseId === focus.shiftNurseId)!.wardShiftList[
                    focus.day
                ] === shiftTypeId
            )
                return;

            const current = shift.divisionShiftNurses.flatMap((x) => x).find((x) => x.shiftNurse.shiftNurseId === focus.shiftNurseId)!
                .wardShiftList[focus.day];
            const request = shift.divisionShiftNurses.flatMap((x) => x).find((x) => x.shiftNurse.shiftNurseId === focus.shiftNurseId)!
                .wardReqShiftList[focus.day];

            if (request != null && request === current && !confirm('신청 근무입니다 정말 바꾸시겠습니까?')) return;
    ...
    const pasteShift = async () => {
        ...
        try {
            ...
            const startNurseIndex = flatNurses.findIndex((x) => x.shiftNurse.shiftNurseId === focus.shiftNurseId);
            ...
            for (let i = 0; i < shiftNameTable.length; i++) {
                ...
                for (let j = 0; j < shiftNameTable[i].length; j++) {
                    ...
                    wardShiftsDTO.push({
                        ...
                        date: `${year}-${month.toString().padStart(2, '0')}-${(focus.day + j + 1).toString().padStart(2, '0')}`,
    ...
    useEffect(() => {
        if (activeEffect && wardConstraint) setState('checkFaultOptions', updateCheckFaultOption(wardConstraint));
    ...
    return {
        ...
        state: {
            ...
            readonly,
            showLayer,
            currentShiftTeam: shiftTeams?.find((x) => x.shiftTeamId === currentShiftTeamId) as ShiftTeam | undefined,
            shiftTeams,
            postShiftLoading,
        },
        actions: {
