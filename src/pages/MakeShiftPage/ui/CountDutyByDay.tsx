...
function CountDutyByDay() {
    ...
    return (
        shift && (
            <div id="count_by_day" className="rounded-[1.25rem] bg-[#FDFCFE] shadow-[0rem_-0.25rem_2.125rem_0rem_#EDE9F5]">
                {shift.wardShiftTypes
                    ...
                    .map((wardShiftType, index) => (
                        ...
                        >
                            <div
                                className={`flex h-full w-12.5 items-center justify-center font-poppins text-[1.5rem] ${index === 0 && 'rounded-tl-[1.25rem]'} ${
                                    ...
                                    >
                                        {
                                            shift.divisionShiftNurses
                                                .flatMap((rows) => rows)
                                                .filter((row) => row.wardShiftList[i] === wardShiftType.wardShiftTypeId).length
                                        }
                                    </p>
                                ))}
                            </div>
