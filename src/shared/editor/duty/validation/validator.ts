import type {CellPos} from '../../editor-core/selection';
import type {Validator, Violation} from '../../editor-core/validation';
import type {DutyDoc} from '../doc';
import type {DutyValidationInput} from './types';

export type DutyRuleKey =
    | 'maxContinuousWork'
    | 'minNightInterval'
    | 'maxContinuousNight'
    | 'minContinuousNight'
...
export type DutyRuleDefinition = {
    key: DutyRuleKey;
    isActive: boolean;
    regExp: RegExp;
    message: string;
 ...
 */
export function createDutyValidator(input: DutyValidationInput): Validator<DutyDoc> {
...
export function getDutyRuleDefinitions(input: DutyValidationInput): DutyRuleDefinition[] {
...
function buildRowString(doc: DutyDoc, row: number, requestedOffByRow?: boolean[][]): string {
...
function rangeToCells(row: number, startCol: number, endCol: number, colCount: number): CellPos[] {
